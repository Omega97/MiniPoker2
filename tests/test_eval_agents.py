...
def main(game_power=4, deck_size=52, n_games=200_000):
    # Setup
    ...
    agent = BatchKernelSmoothedMethodicalAgent(game, epochs=1_000, lr=0.03, rollout_samples=3,
                                                    explore_proba=0.01, max_sigma=10.)
    trainer = AgentTrainer(agent)
    trainer.run()
    agents.append(agent)

    agent_psa = PosteriorSamplingAgent(game, epochs=1000, lr=0.01, rollout_samples=1,
                                       explore_proba=0.01, max_sigma=5.)
    trainer = AgentTrainer(agent_psa)
    trainer.run()
    agents.append(agent_psa)

    agent_2 = PosteriorSamplingAgent(game, epochs=1000, lr=0.01, rollout_samples=1, explore_proba=0.01, max_sigma=2.)
    agent_2.inherit_from(agent_psa)
    trainer = AgentTrainer(agent_2)
    trainer.run()
    agents.append(agent_2)

    agent_3 = PosteriorSamplingAgent(game, epochs=1000, lr=0.001, rollout_samples=10, explore_proba=0.01, max_sigma=2.)
    agent_3.inherit_from(agent_2)
    trainer = AgentTrainer(agent_3)
    trainer.run()
    agents.append(agent_3)

    agent = CEMAgent(game, epochs=100, lr=0.1, rollout_samples=10, explore_proba=0.01, max_sigma=5.)
    agent.set_compare_agent(good_agent)
    trainer = AgentTrainer(agent)
    trainer.run()
    agents.append(agent)
